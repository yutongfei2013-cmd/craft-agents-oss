...
Craft Agents is an open-source AI agent desktop app built by Craft Docs Ltd. It provides a GUI-based multi-session agent experience powered by Codex Agent SDK and Pi SDK, supporting multiple LLM providers (Anthropic, Google AI Studio, ChatGPT/Codex, GitHub Copilot, OpenRouter, Ollama, etc.).
...
| `session-tools-core` | @craft-agent/session-tools-core | Shared utilities for session-scoped tools (Codex and Codex) |
...
- **Codex backend** — `ClaudeAgent` class in `packages/shared/src/agent/Codex-agent.ts`, powered by `@anthropic-ai/Codex-agent-sdk`. Handles Anthropic API key, Codex Max/Pro OAuth, and all third-party endpoints (OpenRouter, Ollama, custom).
...
- **Provider abstraction**: Main process code (`src/main/`) must NOT import provider backends directly (`Codex-agent`, `codex-agent`, `copilot-agent`, `pi-agent`, `@github/copilot-sdk`). Use provider-agnostic APIs from `@craft-agent/shared/agent/backend`.
